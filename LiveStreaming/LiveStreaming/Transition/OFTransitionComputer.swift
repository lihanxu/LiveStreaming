...
/// 设置页上的一项转场预设。
struct OFTransitionPreset {
    /// 格子 / 选项标题
    let displayName: String
    /// 对应 GPU 模版；nil 表示关
    let style: OFTransitionStyle?
    
    /// 内置列表，第一项为关闭
    static let all: [OFTransitionPreset] = [
...
class OFTransitionComputer: NSObject, OFProcessNode {
    /// 共享 Metal 上下文
    ...
    /// 当前选中的预设
    var currentPreset: OFTransitionPreset {
    ...
    func setDurationSlider(_ value: Float) {
        stateLock.lock()
