...
public struct OFCartoonPreset {
    ...
    /// 内置预设，第一项为关闭
    public static let all: [OFCartoonPreset] = [
...
/// 处理图中的整帧漫画风节点。
public class OFCartoonComputer: NSObject, OFProcessNode {
    /// AnimeGANv3 固定输入边长
    ...
    /// 当前选中的预设
    public var currentPreset: OFCartoonPreset {
        lock.lock()
    ...
    private func locateModel(named: String) -> URL? {
        if let compiled = OFFilterResources.url(forResource: named, withExtension: "mlmodelc") {
            return compiled
        }
        guard let raw = OFFilterResources.url(forResource: named, withExtension: "mlmodel") else {
            return nil
        }
        do {
